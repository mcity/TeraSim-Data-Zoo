...
class _Junction:
    ...
    def plot(self, ax, **kwargs):
        ...
        if self.shape is not None:
            ...
            try:
                coord2d = [(x, y) for x, y,_ in self.shape.boundary.coords]
            except:
                coord2d = [(x, y) for x, y in self.shape.boundary.coords]
            poly = matplotlib.patches.Polygon(coord2d, closed = True, **kwargs)
            poly.sumo_object = self
            ax.add_patch(poly)
            return poly
